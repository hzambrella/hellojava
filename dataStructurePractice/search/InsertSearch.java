...
//插值查找
//二分查找是mid=(begin+end)/2      begin-begin/2+end/2   begin+(end-begin)*1/2
//插值查找则是  mid=begin+(end-begin)*(key-begin_val)/(end_val-begin_val)
public class InsertSearch <T extends Comparable<T>>{
	...
	public int search(T[] tosearch,T target){
		...
		int mid=begin+(end-begin)*(target.compareTo(tosearch[begin]))/(tosearch[end].compareTo(tosearch[begin]));
